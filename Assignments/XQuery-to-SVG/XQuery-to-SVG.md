...
We next wanted to test how date arithmetic works. We defined `$ldt-one` and `$ldt-two` to return the first and second values in our sorted sequence of dateTimes. We then returned a string-joined list showing us the first and second dates, and then performed simple subtraction with a minus sign. When we `eval` this in eXide we see:
