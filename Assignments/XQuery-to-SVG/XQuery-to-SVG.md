...
This tutorial is designed to show you how to work with XQuery in eXist-db to generate an SVG visualization. This will also introduce you to global declarations and user-defined functions in XQuery, as we follow a project that needs to do date and duration arithmetic to plot a timeline from XML data coded with `xs:dateTime` and `xs:duration` datatypes.
...
Launch dates are encoded in the Rocket Launches project using the `xs:dateTime` datatype in an `@sDateTime` attribute on the `<launch>` element. We accessed the launch dateTimes in the Rocket Launches project with this global variable: 

```
declare variable $launchDateTimes as xs:dateTime+ := $rocketColl//launch/@sDateTime ! xs:dateTime(.) => sort();
```
Notice that we explicitly identified a datatype for the variable sequence to the left of the `:=` and we use a repetition indicator to show there will be *one or more* `xs:dateTime` values in this variable. To the right of the `:=` we make sure it is actually output as that datatype instead of as an attribute node, by using simple map with `! xs:dateTime(.)`. Finally, we use the arrow operator `=>` to send the sequence of values to the XPath `sort()` function to organize them from earliest to latest, since we know that `xs:dateTime` values are sortable in XPath just like integers, decimals, and strings. (If we did not specify the datatype, though, the `sort()` function might interpret the values as strings, using alphabetic sorting, which is not what we want.)   
...
Because we are interested in sorting values and doing date arithmetic, we wanted to see how some simple mathematic operations work on dateTime data in exist-dB. To experiment, we wrote a simple XQuery script which you can find at `/db/2020_ClassExamples/rocketSimpleDateArithmetic.xql`. It looks like this:
...
This XQuery reaches into the Rocket project collection and defines `$launchDateTimes` as a sorted sequence of all the values of `@sDateTime`. Just as with our global variable above, we needed to make sure the XQuery processor recognized these as xs:dateTime, so we mapped each value as such with `xs:dateTime(.)`, and we then applied the `sort()` function, which should sort the dates from earliest to latest. 
...
The third value here, `P214DT3H9M56S`, is an **xs:duration** datatype indicating that the difference between November 12, 1981 at 7:00:03 and April 12, 1981 at 10:09:59 is 214 days, 3 hours, 9 minutes, and 56 seconds. As a further experiment we tried to see what would happen if we changed the minus to a plus, to *add* the two dates. This returned an error message that indicated the operand after the plus sign should be a duration of some kind, not a date. (This makes sense if you think about it. We would commonly do date arithmetic using subtraction to find out how far apart in time two dates are. Addition would only be meaningful in the context of adding a duration to a date, to find out, for example, what will be the date 90 days from now?) 

We share this just to inform you of how `xs:dateTime` and `xs:duration` are commonly processed in the universe of XML data. We also need to be aware of the limitations of that processing. If we want to draw SVG lines and shapes that indicate relative proportion and distance in time, SVG will not be able to read an `xs:duration` value or an `xs:dateTime` value directly. These datatypes will need to be converted to a decimal format (`xs:decimal`) in order to plot them on a screen. 

In a perfect universe, there would be a ready-made function to convert the date and duration datatypes to a decimal notation. However, the documentation and our own experience shows us that it’s only possible for XPath to do date arithmetic within the family of dateTime and duration datatypes (including `gYear`, `gYearMonth`, etc). We cannot convert those into decimal values automatically with something like `number()`, `xs:decimal()`, `xs:integer()`, or `xs:float()`. (Try it yourself and take a look at the error messages.) We are going to have to define our own functions to create a reasonable conversion of our dateTime and duration values into a decimal notation for plotting shapes on a screen. 
...
Within our XQuery that is processing data from the Rocket Launches project, we can now call our new user-defined function. We will be doing that in multiple places, wherever we need to convert an `xs:duration` value into an `xs:decimal` value. Here is one such place in the code:
...
```
We are continuing our practice of *typing* the variables with `as` statements. The first variable, `$duration` extracts a duration value, and the next variable, `$durDayDec`, sends that duration value to our user-defined `ebb:durationConverter()` function, bundling `$duration` inside its parentheses as the parameter to be delivered for processing.


#### Converting xs:dateTime into an xs:decimal
Our durationConverter function will do the work of preparing decimal values from duration data, and we have multiple uses for it. As we discussed in [the earlier section on date arithmetic](#Into-the-weeds-of-date-arithmetic), when we subtract the `xs:dateTime` value of an early date from a later date, we return an `xs:duration` value in days. Knowing this, we can we can convert the dateTime value of each mission launch from the Rocket Project collection and convert it into a duration value. 

We will do that by working with the earliest of the dateTime values, the **minimum** value in the sequence of dateTimes. This is useful static information, so we stored it in our section of global variables as our “ground zero” plotting the timeline. We also stored the latest date to help determine endpoint of the timeline: 
...
```
Our XQuery code will need to loop through the sequence of `$launchDateTimes` in order to report and visualize information about each one. Here is our `for-loop`:   
```
for $l in $launchDateTimes
```
As we mentioned earlier, each `$l` dateTime will need to be converted to a decimal value based on its difference in days from the earliest mission. It is time to create another user-defined function to do this work. (We typically define these functions in their own section near the top of the document near the other global declarations, and we bracket them off with an XQuery comment `(: .... :) ` to explain what they are for.)
...
```
Notice that this function takes one input parameter in the `xs:dateTime` datatype and returns `xs:decimal` output. First, it does a simple subtraction: the current input dateTime minus the earliest dateTime, which, in the way of XPath date arithmetic returns an `xs:duration`. The next line invokes our first user-defined function, the durationConverter, to return a decimal value. 
...
Here is a simple text output from our first XQuery designed to survey the data conversions from our two user-defined functions. 
```
...
```
Now that we have prepared our data in decimal notation, we are ready to visualize it.  
...
$ThisFileContent
```
Here we declared the global variable $ThisFileContent (so that its declaration ends with a semicolon after the the close tag of the `</svg>`. (You will remember and find on other examples how to use this variable with others to store the output document to a directory you designate on our eXist-dB.) In the way we set this up from the start, we regularly return `$ThisFileContent` in the eXide results window by just entering it as the last line of the file. This is how you “call” a global variable, and how we recommend testing and viewing the SVG output as you are creating it.
...
In our `for-loop`, we will need to subtract the `min()` value from each mission dateTime decimal value, so we can plot from 0 (as the earliest launch). And just like we did with the `max()` value, we want to multiply it by our spacer (our `$timelineSpacer` variable, and adjust that spacer value so our plot is easy to read (neither too crowded nor too gappy). We will output the actual dateTime and the mission identifier as labels for our data using SVG `<text>`, and we'll use the decimal duration value each mission to determine the relative size or area of the shape we wish to plot.

#### Namespace issues and Q{}
As soon as we create an SVG document in XQuery, we are placing the new code we generate in the SVG namespace. That means when writing an XQuery script inside the SVG root element, we will not be able to extract project data unless we indicate that it is in another namespace or no namespace. We indicate the absence of a namespace using a special notation: `Q{}` set before any elements in an XPath reaching into project data. Attributes by default are not referenced in a namespace so you don’t have to indicate anything special aobut them, but you do need to use the `Q{}` notation on any element nodes from the Rocket Launches collection. Here is how we had to change our variable to read the Rocket Launches data from within our XQuery script.
...
You should be able to plot the timeline now! Run your results with the Eval button, and view them as **Adaptive Output** in the results window to look at your code. You should see SVG generated with its namespace in the root node, and you should see a simple SVG file containing a line element. You can view the SVG as a graphic in XQuery by toggling the XML Output option to **Direct Output**, but you will probably need to scroll to see your entire line, and you may not be able to scroll. That is because we need to set the width and height attributes on our SVG and set up the long vertical line to be viewable in a browser window on scrolling down.

#### Setting the viewport and shifting things with transform="translate(x, y)" so you can see the full line:

To make your long line visible, you want to estimate something wider than its widest x value and something a little longer than its largest y value so that you program a viewable area for your SVG. When generating SVG with calculated values as we are doing, this can be tricky, so we usually output our code first and read its maximum values before plugging in what is known as the viewport. To create a viewport, you need to add `@width` and `@height` attributes to your `<svg>` root element. We did this in our SVG timeline by using raw numbers without units, estimating a bit beyond our largest y value and our widest x value, thinking about how wide we will eventually want to make our file.

We also decided to shift our SVG over a little bit so that if we use (0, 0) coordinates for the start of the line, the timeline won’t be flush against the top and side of the screen. To do this, we add `transform="translate(x, y)"` to the outermost `<g>` element, which bundles the all the SVG elements into a group. Using the `transform="translate(x, y)"`function, we adjust the x and y values of the plotted elements inside, moving them over by x units and up or down by y units. 

We have created a sample starter XQuery to SVG file for this exercise and stored it in our newtFire eXist-dB as `/db/2020_ClassExamples/rocketTimelineStarter.xql`. We have also posted [a sample output timeline](http://newtfire.org:8338/exist/rest/db/2020_ClassExamples/rocketTimeline-sample.svg) which could stand some improvement in spacing and style. We are sure that you can create something more legible and attractive than this. Your task now is to continue developing our starter XQuery file that we saved in the Newtfire eXist-dB and prepare a complete, labeled, and legible SVG plot.