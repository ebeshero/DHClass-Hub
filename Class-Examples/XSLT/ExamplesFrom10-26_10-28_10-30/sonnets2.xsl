...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <html>
            ...
            <body>
                ...
                <ul>
                    <xsl:apply-templates select="$sonnetColl//sonnet" mode="toc">
                        <xsl:sort order="descending"/><!-- this is sorting alphabetically and the @order value "descending" indicates we want it to go from z-a instead of a-z -->
                    </xsl:apply-templates> <!-- the "$" references the defined variable from above and the @mode is how we use modal XSLT to select the same part of the source document multiple times in order to write different template rules grabbing from the same selected part of the source document -->                        
                </ul>
                <hr/>
    ...
    <xsl:template match="sonnet" mode="toc">
        <li>
            <a href="#sonnet{@number}"> <!-- here we are using an Attribute Value Template to set the attribute value of @number as the new attribute value for @href (notice the curly braces) creating a link between the table of contents and the titles of each sonnet later in the body-->
